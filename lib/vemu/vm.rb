module Vemu
  class VM
    ...
    def invoke_qemu
      ...
      `qemu-img create -f qcow2 -F qcow2 -b '#{base_image_path}' '#{File.join @vm_path, 'diffdisk'}' #{@disk}`
    ...
    def qemu_cmd_arguments
      ...
      storage_args = [
        "-drive", "file=#{File.join @vm_path, 'diffdisk'},if=virtio,discard=on,cache=unsafe"
      ...
      cloudinit_args = [
        "-drive", "id=cdrom0,if=none,format=raw,readonly=on,file=#{File.join @vm_path, 'cidata.iso'}",
      ...
      other_args = [
        ...
        "-parallel", "none",
        "-chardev", "socket,id=char-serial,path=#{File.join @vm_path, 'serial.sock'},server=on,wait=off,logfile=#{File.join @vm_path, 'serial.log'}",
        "-serial", "chardev:char-serial",
        "-chardev", "socket,id=char-serial-virtio,path=#{File.join @vm_path, 'serialv.sock'},server=on,wait=off,logfile=#{File.join @vm_path, 'serialv.log'}",
        "-device", "virtio-serial-pci,id=virtio-serial0,max_ports=1",
        "-device", "virtconsole,chardev=char-serial-virtio,id=console0",
        "-chardev", "socket,id=char-qmp,path=#{File.join @vm_path, 'qmp.sock'},server=on,wait=off",
        "-qmp", "chardev:char-qmp",
        "-name", @name,
        "-pidfile", File.join(@vm_path, 'qemu.pid'),
      ]

      (machine_args + storage_args + cloudinit_args + network_args + other_args).map(&:to_s)
    end
